interface ObservableConfig {
  ...
  observers: any[];
  paused: boolean;
  path: { property: string; target: any }[];
...
const ObservableSlim = (function () {
  ...
  const observables: ObservableConfig[] = [];
  // An array of all the objects that we have assigned Proxies to
  const targets: {}[] = [];

  // An array of arrays containing the Proxies created for each target object. targetsProxy is index-matched with
  // 'targets' -- together, the pair offer a Hash table where the key is not a string nor number, but the actual target object
  const targetsProxy: Array<Array<object>> = [];
  ...
  const _getProperty = function<T1, T2>(obj: T1, path: string): any {
  ...
  /*	Function: _create
      ...
      Returns:
        An ES6 Proxy object.
  */
  const _create = function (target: object, domDelay: boolean, originalObservable: ObservableConfig, originalPath: ObservablePath[]) {

    let observable: ObservableConfig = originalObservable || null;

    // record the nested path taken to access this object -- if there was no path then we provide the first empty entry
    const path: ObservablePath[] = originalPath || [{'target': target, 'property': ''}];
    paths.push(path);
    ...
    */
    const _getPath = function (target: object, property: string, jsonPointer?: boolean): string {
