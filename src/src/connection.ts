...
import {Clock, ClockImmutable, DocIdToClockImmutable} from './clock.interface';
...
function clockUnion(clockMap: DocIdToClockImmutable, docId: string, clock: ClockImmutable): any {
  clock = clockMap.get(docId, Map()).mergeWith((x, y) => Math.max(x as number, y as number), clock);
...
class Connection<T> {
  private _theirClock: DocIdToClockImmutable;
  private _ourClock: DocIdToClockImmutable;
  private _docChangedHandler: OmitThisParameter<(docId: string, doc: Doc<T>) => void>;

  constructor (
    private _docSet: DocSet<T>,
    private _sendMsg: MessageSender,
    ) {
  ...
  sendMsg (docId: string, clock: ClockImmutable, changes?: Change[]): void {
  ...
  // Callback that is called by the dOmitThisParameter<(docId, doc) => void>ocSet whenever a document is changed
  docChanged(docId: string, doc: Doc<T>): void {
    ...
    const docIdClock = this._ourClock.get(docId, Map());

    if (!lessOrEqual(docIdClock, clock)) {
