...
import {IHasOBJECT_ID} from '../frontend/constants';
...
interface TestBookRow {
  authors: Array<string>,
  title: string,
  isbn: string,
}

interface TestTableDoc {
  books: Table<TestBookRow & IHasOBJECT_ID, []>
}
...
describe('Automerge.Table', () => {
  describe('Frontend', () => {
    it('should generate ops to create a table', () => {
      const actor = uuid();
      const [doc, req] = Frontend.change(Frontend.init(actor), (doc: any) => {
    ...
    it('should generate ops to insert a row', () => {
      const actor = uuid();
      const [doc1, req1] = Frontend.change(Frontend.init(actor), (doc: any) => {
        doc.books = new Automerge.Table(['authors', 'title'])
      });
      let rowId;
      const [doc2, req2] = Frontend.change(doc1, (doc: any) => {
  ...
  describe('with one row', () => {
    ...
    beforeEach(() => {
      s1 = Automerge.change(Automerge.init({freeze: true}), (doc: any) => {
    ...
    it('should be immutable', () => {
      assert.strictEqual((s1.books as any).add, undefined);
      assert.throws(() => s1.books.set('id', {} as any), /can only be modified in a change function/);
    ...
    it('should allow a row to be updated', () => {
      const s2 = Automerge.change(s1, (doc: any) => {
  ...
  it('should allow concurrent row insertion', () => {
    const a0 = Automerge.change(Automerge.init(), (doc: any) => {
      doc.books = new Automerge.Table(['authors', 'title', 'isbn'])
    });
    const b0 = Automerge.merge(Automerge.init(), a0);

    let ddia, rsdp;
    const a1 = Automerge.change(a0, (doc: any) => { ddia = doc.books.add(DDIA) });
    const b1 = Automerge.change(b0, (doc: any) => { rsdp = doc.books.add(RSDP) });
  ...
  it('should allow rows to be sorted in various ways', () => {
    const s = Automerge.change(Automerge.init(), (doc: any) => {
      doc.books = new Automerge.Table(['authors', 'title', 'isbn']);
      doc.books.add(DDIA);
      doc.books.add(RSDP)
    });
    ...
    assert.deepEqual(s.books.sort((row1: TestBookRow, row2: TestBookRow) => {
